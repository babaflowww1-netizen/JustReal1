...
def init_db():
    ...
    try:
        admin_mail = "justreal07.admin@justreal-secure.net"
        admin_pass = "JustReal2026*"
        admin_user = "JustReal07"
        cursor.execute('''
        ...
        cursor.execute("UPDATE users SET role = 'Admin', password = ?, daily_limit = 999999 WHERE username = 'JustReal07'", (admin_pass,))
        conn.commit()
    except:
        pass
...
def telegram_bot_listener():
    ...
    offset = 0
    bot_username = "JustRealCheckerBot"
    try:
        me_res = requests.get(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/getMe", timeout=3).json()
        if me_res.get("ok"):
            bot_username = me_res["result"].get("username", bot_username)
    except:
        pass
        ...
        try:
            ...
            if data.get("ok"):
                for result in data.get("result", []):
                    ...
                    # Callback Query (Katıldım Butonu Kontrolü)
                        ...
                        if callback_data == "check_membership":
                            ...
                            if is_member:
                                ...
                                welcome_msg = (
                                    ...
                                    "🚀 **JustReal Multi-Checker** sistemine tam erişim hakkı kazandınız. Paneli açmak için aşağıdaki butonu kullanabilirsiniz."
                                )
                                keyboard = {
                                    "inline_keyboard": [
                                ...
                                requests.post(answer_url, json={"callback_query_id": query_id, "text": "❌ Henüz kanala katılmamışsınız! Lütfen önce kanala katılın.", "show_alert": True})
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        body::before {
            ...
            background-image: url('https://images.unsplash.com/photo-1559526324-4b87b5e36e44?q=80&w=1000&auto=format&fit=crop'), radial-gradient(circle, rgba(139,92,246,0.15) 0%, rgba(11,15,25,0.9) 80%);
        ...
        .top-right-logo { position: absolute; top: 12px; right: 15px; font-weight: 900; color: #8b5cf6; font-size: 18px; letter-spacing: 0.5px; z-index: 10; text-shadow: 0 2px 10px rgba(139,92,246,0.4); }
        
        .card { background: var(--card-bg); backdrop-filter: blur(10px); padding: 16px; border-radius: 14px; margin-bottom: 14px; box-shadow: 0 8px 25px rgba(0,0,0,0.25); border: 1px solid var(--border-col); }
        ...
        button { background: #8b5cf6; font-weight: bold; cursor: pointer; transition: 0.2s; border: none; color: #fff; box-shadow: 0 4px 12px rgba(139,92,246,0.3); }
        ...
        .avatar-img { width: 50px; height: 50px; border-radius: 50%; object-fit: cover; border: 2px solid #8b5cf6; flex-shrink: 0; box-shadow: 0 0 12px rgba(139,92,246,0.5); }
        ...
        {% if not session.get('user') %}
            <div class="card">
                ...
                <form method="POST" action="/auth">
                    ...
                    <input type="text" name="username" placeholder="JustReal07" required>
...
def cc_cleaner():
    ...
    <div style="width:100%; max-width:480px; margin:20px auto; background:var(--input-bg); padding:20px; border-radius:10px; color:var(--text-color); font-family:sans-serif; border:1px solid rgba(59,130,246,0.3);">
        ...
        {% if request.method == 'POST' %}
            <label style="font-size:13px; margin-top:10px; display:block;">Temizlenmiş Liste:</label>
...
def profile():
    ...
    <div style="width:100%; max-width:450px; margin:20px auto; background:var(--input-bg); padding:20px; border-radius:10px; color:var(--text-color); font-family:sans-serif; border:1px solid rgba(99,102,241,0.3);">
        ...
        <div style="text-align:center; margin:15px 0;">
            <img src="__PIC__" style="width:70px; height:70px; border-radius:50%; object-fit:cover; border:2px solid #8b5cf6;" alt="Avatar">
        </div>
        ...
        <div style="background:rgba(139,92,246,0.1); padding:10px; border-radius:6px; margin-bottom:15px; font-size:13px; display:flex; justify-content:space-around; text-align:center;">
            <div><b>__T_CHECKS__</b><br><span style="font-size:11px; opacity:0.7;">Toplam Check</span></div>
            <div><b>__T_LIVES__</b><br><span style="font-size:11px; opacity:0.7;">Toplam Live</span></div>
            <div><b>%__SUCCESS_RATE__</b><br><span style="font-size:11px; opacity:0.7;">Başarı Oranı</span></div>
        </div>
        
        <form method="POST">
            <label style="font-size:13px;">Kullanıcı Adı:</label>
            <input type="text" name="username" value="__UNAME__" required style="width:100%; padding:10px; margin:4px 0; background:rgba(255,255,255,0.05); color:inherit; border:1px solid rgba(255,255,255,0.1); border-radius:6px;">
            
            <label style="font-size:13px;">Gmail Adresi:</label>
            <input type="email" name="email" value="__UEMAIL__" required style="width:100%; padding:10px; margin:4px 0; background:rgba(255,255,255,0.05); color:inherit; border:1px solid rgba(255,255,255,0.1); border-radius:6px;">
            
            <label style="font-size:13px;">Avatar URL:</label>
            <input type="url" name="profile_pic" value="__PIC__" required style="width:100%; padding:10px; margin:4px 0; background:rgba(255,255,255,0.05); color:inherit; border:1px solid rgba(255,255,255,0.1); border-radius:6px;">
            ...
            <select name="theme" style="width:100%; padding:10px; margin:4px 0; background:rgba(255,255,255,0.05); color:inherit; border:1px solid rgba(255,255,255,0.1); border-radius:6px;">
                <option value="dark" __DARK_SEL__>Koyu Tema (Dark Mode)</option>
                <option value="light" __LIGHT_SEL__>Açık Tema (Light Mode)</option>
            </select>
    ...
    profile_html = profile_html.replace("__PIC__", u['profile_pic'])
    profile_html = profile_html.replace("__T_CHECKS__", str(t_checks))
    profile_html = profile_html.replace("__T_LIVES__", str(t_lives))
    profile_html = profile_html.replace("__SUCCESS_RATE__", str(success_rate))
    profile_html = profile_html.replace("__UNAME__", u['username'])
    profile_html = profile_html.replace("__UEMAIL__", u['email'])
    profile_html = profile_html.replace("__DARK_SEL__", "selected" if u.get('theme')=='dark' else "")
    profile_html = profile_html.replace("__LIGHT_SEL__", "selected" if u.get('theme')=='light' else "")
...
def admin():
    ...
    alert_box = f'<div style="background:rgba(16, 185, 129, 0.2); border:1px solid #10b981; padding:8px; border-radius:6px; margin-bottom:10px; color:#6ee7b7; font-size:13px; text-align:center;">{success_msg}</div>' if success_msg else ''
    admin_html = admin_html.replace("__SUCCESS_ALERT__", alert_box)
    admin_html = admin_html.replace("__LOGS_ROWS__", logs_html if logs_html else '<tr><td colspan="5" style="text-align:center; padding:10px; font-size:12px;">Log bulunamadı.</td></tr>')
