...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .wide-check-card { width: 100%; margin-left: 0; padding: 18px; background: var(--card-bg); border-radius: 14px; border: 1px solid rgba(139, 92, 246, 0.4); box-shadow: 0 10px 30px rgba(0,0,0,0.3); }
        .wide-check-card textarea { width: 100%; min-height: 110px; resize: vertical; }
        @media (min-width: 768px) { body { padding: 30px; } .container { max-width: 580px; } }
...
def auth():
    ...
    if action == 'register':
        try:
            ...
            cursor.execute('INSERT INTO users (username, email, password, profile_pic, role, daily_limit, total_lives, total_checks, is_banned, theme) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, ?)',
                           (input_val, f"{input_val}@user.com", password, default_avatar, 'Normal', 1500, 0, 'dark'))
            conn.commit()
        except sqlite3.IntegrityError:
            conn.close()
            return redirect(url_for('index', error="Bu Kullanıcı Adı veya E-posta zaten kayıtlı!"))
    ...
    return redirect(url_for('index', error="Giriş Başarısız! Kullanıcı adı/posta veya şifre hatalı."))
...
def profile():
    ...
    <div style="width:100%; max-width:450px; margin:20px auto; background:var(--input-bg); padding:20px; border-radius:10px; color:var(--text-color); font-family:sans-serif; border:1px solid rgba(99,102,241,0.3);">
        ...
        <form method="POST">
            ...
            <select name="theme" style="width:100%; padding:10px; margin:4px 0;">
                <option value="dark" {'selected' if u.get('theme')=='dark' else ''}>Dark</option>
                <option value="light" {'selected' if u.get('theme')=='light' else ''}>Light</option>
            </select>
    ...
    """
    return render_template_string(p_html, msg=msg)
